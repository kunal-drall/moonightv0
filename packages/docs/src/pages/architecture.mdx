...
Moonight supports two wallet connection paths through a unified interface:
...
│  │  (Starkzap SDK)  │  │  (starknet-react)│  │
│  │                  │  │                  │  │
│  │  Google          │  │  Braavos         │  │
│  │  Twitter         │  │  Ready Wallet    │  │
│  │  Email/Passkeys  │  │                  │  │
│  └────────┬─────────┘  └────────┬─────────┘  │
│           │                     │             │
│           ▼                     ▼             │
...
- **Social Login**: Powered by [Starkzap SDK](https://starkzap.dev/) + Cartridge Controller. No browser extension needed. Supports gasless (sponsored) transactions.
- **Wallet Apps**: Traditional browser wallets via `starknet-react`. Users install Braavos or Ready Wallet extension.
- **Unified Hook**: `useUnifiedWallet()` returns a normalized interface. Transaction code branches on `source` to use the appropriate API (starkzap `TxBuilder` vs starknet.js `account.execute`).
